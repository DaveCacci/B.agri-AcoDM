...
<p> Last review: <em> December 06, 2020 </em></p></html>"));
  ...
  record ParametersADM1_physiochemical
    ...
    constant Real deltaH0_Ksp_stru(unit = "J/mol") = 0 "enthalpy of reaction of struvite formation. Rontenltap et al 2007 as 22.6 kJ/mol; ; 98.82 kJ/mol; Struvite precipitation is endo up to 30°C and eso between 30-60°C";
    ...
    parameter Real pKsp_stru(unit = "conc") = 13.26 "New value from PCM? Also Ronteltap et al. 2007";
...
<p> Last review: <em> December 06, 2020 </em></p></html>"));
  ...
  record StateVariablesADM1
    ...
    Real S_va(fixed = true, start = 0.00564218) "8/10**";
    Real S_bu(fixed = true, start = 0.00856152) "8/10**";
    Real S_pro(fixed = true, start = 0.0461522) "8/10**";
    Real S_ac(fixed = true, start = 0.880519) "8/10**";
    Real S_vam(fixed = true, start = 0.00559991) "8/10";
    Real S_bum(fixed = true, start = 0.00856152) "8/10";
    Real S_prom(fixed = true, start = 0.0457902) "8/10";
    Real S_acm(fixed = true, start = 0.0572828) "8/10";
    //Inorganic soluble components
    Real S_h2(fixed = true, start = 4.11532e-07);
    Real S_ch4(fixed = true, start = 0.0474429);
    Real S_ic(fixed = true, start = 0.0563627) "8/10";
    Real S_in(fixed = true, start = 0.0589474) "8/10";
    Real S_ip(fixed = true, start = 0.00939197) "8/10";
    Real S_cat(fixed = true, start = 0.00203517);
    Real S_ca(fixed = true, start = 0.00058497);
    Real S_mg(fixed = true, start = 0.00203739);
    Real S_an(fixed = false, start = 0.000341016);
    Real S_hco3(fixed = true, start = 0.0466956) "8/10";
    Real S_nh3(fixed = true, start = 0.000707317) "8/10";
    Real S_nh4(fixed = true, start = 0.0582401) "8/10";
    Real S_hpo4(fixed = true, start = 0.0036366) "8/10";
    Real S_h2po4(fixed = true, start = 0.00575536) "8/10";
    Real S_po4(fixed = true, start = 1.22302e-08) "8/10";
    Real S_co2(fixed = true, start = 0.00966706) "8/10";
    Real pH(fixed = false, start = 7.11);
    Real S_h(fixed = true, start = 1.04198e-07) "8/10";
    Real S_oh(fixed = true, start = 2.2964e-07) "8/10";
    Real Theta(fixed = true, start = 1.25422e-07);
    //Gas components
...
//MMP submodel
    ...
    //  rp2 = (1+sign(SI_stru))/2 * kcryst_stru * X_stru * ((a_nh4 * a_po4 * a_mg / Ksp_stru) ^ (1 / 3) - 1) ^ 3;
      rp1=0;
      rp2=0;
      //rp2 = (1+sign(SI_stru))/2*kcryst_stru * X_stru / (Ksp_stru)^(3/3) * ((a_nh4 * a_po4 * a_mg) ^ (1 / 3) - (Ksp_stru)^(1/3)) ^ 3;
    //  SI_acp=0;
    //SI_stru=0;
    //  rp1=0;
    //  rp2=0;
...
//Cations and anions
    ...
    //  Theta = -(S_cat + S_nh4 + 2 * S_ca + 2 * S_mg - S_hco3 - S_acm / Ac_conv - S_prom / Pro_conv - S_bum / Bu_conv - S_vam / Va_conv - S_an - 2 * S_hpo4 - S_h2po4 - 3 * S_po4-S_oh);
