...
<p> Last review: <em> January 22, 2021 </em></p></html>"));
  ...
  package Interfaces
    connector Feed_stream_vec
      ...
      stream Modelica.SIunits.MassConcentration X_ch[nW + 1];
      stream Real X_pr;
      stream Real X_li;
      stream Real X_su;
      stream Real X_aa;
  ...
  package BlockLibrary
    model Feed_Source_Ari_vec_
      ...
      parameter Boolean Catenacci;
      parameter Integer nXin;
      parameter Integer nW = 1;
      parameter Integer ID;
      parameter Boolean TKN;
...
// Variabili che "if Catenacci then inlet else calcola"
...
//
      ...
      for i in 1:(nW + 1) loop
        outlet.X_ch[i] = if i == ID or i == nW + 1 then if Catenacci == true then (tCH_in - sCH)*COD_CH_in/1000 - (outlet.X_i[end]*f_xch_xi + outlet.X_c[end]*f_ch_xc + outlet.X_c[end]*f_xi_xc*f_xi_ch_xc) else Xch_in else 0;
      end for;
//Input sempre da TKN
...
//Block variables
    ...
    partial model Digester_stream_onehyd_vec
      ...
      parameter Boolean noXc;
      parameter Boolean multihyd;
      parameter Boolean open;
      Real k_hyd_T[8];
      Real f_nW[nW, 1] "3 columns=CH,PR,LI";
    equation
...
//Inputs
//
      for i in 1:size(f_nW, 1) loop
        f_nW[i, 1] = FeedMix_In.X_ch[i]/FeedMix_In.X_ch[end];
//    f_nW[i,2]= FeedMix_In.X_pr[i]/FeedMix_In.X_pr[end];
//    f_nW[i,3]= FeedMix_In.X_li[i]/FeedMix_In.X_li[end];
      end for;
...
//MPP submodel
      ...
      SI_acp = log(arg_log_acp);
      SI_stru = log(arg_log_stru);
      rp1 = if noEvent(precipitation == false) then 0.0 else (1 + sign(SI_acp))/2*kcryst_acp*X_acp*((a_ca^3*a_po4^2/Ksp_acp)^(1/5) - 1)^2;
      rp2 = if noEvent(precipitation == false) then 0.0 else SI_stru*kcryst_stru*X_stru*((a_nh4*a_po4*a_mg/Ksp_stru)^(1/3) - 1)^2;
      rd2 = if noEvent(precipitation == false) then 0.0 else (1 - sign(SI_stru))/2*kcryst_stru*X_stru*((a_nh4*a_po4*a_mg/Ksp_stru)^(1/3) - 1)^2;
      rd1 = if noEvent(precipitation == false) then 0.0 else (1 - sign(SI_acp))/2*kcryst_acp*X_acp*((a_ca^3*a_po4^2/Ksp_acp)^(1/5) - 1)^2;
...
//Soluble matter
      ...
      24*3600*der(S_in) = if noEvent(noXc == true) then FeedMix_In.Q/V_liq*(FeedMix_In.S_in - S_in) - Y_su*N_bac*r5 + (N_aa - Y_aa*N_bac)*r6 - Y_fa*N_bac*r7 - Y_c4*N_bac*r8 - Y_c4*N_bac*r9 - Y_pro*N_bac*r10 - Y_ac*N_bac*r11 - Y_h2*N_bac*r12 + (N_bac - N_i*f_xi_xb - N_aa*f_pr_xb)*(r13 + r14 + r15 + r16 + r17 + r18 + r19) - (rp2 - rd2) - rt12 else FeedMix_In.Q/V_liq*(FeedMix_In.S_in - S_in) - Y_su*N_bac*r5 + (N_aa - Y_aa*N_bac)*r6 - Y_fa*N_bac*r7 - Y_c4*N_bac*r8 - Y_c4*N_bac*r9 - Y_pro*N_bac*r10 - Y_ac*N_bac*r11 - Y_h2*N_bac*r12 + (N_bac - N_xc)*(r13 + r14 + r15 + r16 + r17 + r18 + r19) + (N_xc - f_xc[4]*N_i - f_xc[5]*N_i - f_xc[2]*N_aa)*r1 - (rp2 - rd2) - rt12;
...
//Inorganic Phosforous Balance
      IP_balance_in = FeedMix_In.Q*(FeedMix_In.S_ip + P_i*(FeedMix_In.S_i + FeedMix_In.X_i[end]) + P_li*FeedMix_In.X_li + P_bac*(FeedMix_In.X_su + FeedMix_In.X_aa + FeedMix_In.X_fa + FeedMix_In.X_c4 + FeedMix_In.X_pro + FeedMix_In.X_ac + FeedMix_In.X_h2[1]));
...
//
    ...
    model Batch
      ...
      Real Xch_start;
    equation
      Xch_start = Digester_stream_onehyd_vec.FeedMix_In.X_ch[end];
      when Xch_start > 1 then
        reinit(Digester_stream_onehyd_vec.X_ch, Xch_start);
      end when;
    end Batch;
...
//  Real S_mg = 0.013 "";
  ...
  equation
    ...
    connect(feed_Source_Ari.inlet[2:end], Input_CattleSlurry.y[1:26]);
    connect(Input_CattleSlurry.y, feed_Source_Ari_vec_.inlet[2:end]) annotation(
      Line(points = {{-174, 20}, {22, 20}, {22, 32}}, color = {0, 0, 127}, thickness = 0.5));
    connect(timeTable.y[1], feed_Source_Ari_vec_.inlet[1]) annotation(
      Line(points = {{-158, 26}, {22, 26}, {22, 32}}, color = {0, 0, 127}, thickness = 0.5));
    connect(feed_Source_Ari_vec_.outlet, sink_vec.inlet) annotation(
      Line(points = {{38, 30}, {82, 30}, {82, 26}}));
  end Test_feed_Ari;
  ...
  model Ari_SS_vec
    ...
    ADM1_P.BlockLibrary.Feed_Source_Ari_vec_ feed_Source_Ari(Catenacci = true, ID = 1, S_an_in = 0.04, TKN = true, f_ch_xc = 0.2, f_li_xc = 0.3, f_pr_xc = 0.2, f_si_xc = 0, nW = nW, nXin = nXin, other_cat_an = 0, steady = false) annotation(
      Placement(visible = true, transformation(origin = {-60, -20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ADM1_P.BlockLibrary.Feed_Source_Ari_vec_ feed_Source_Ari_vec_(Carbonates_in = 0, Catenacci = true, Flores = true, ID = 2, S_an_in = 0.04, TKN = false, f_Xc_in = 0.0, f_ch_xc = 0.2, f_li_xc = 0.3, f_pr_xc = 0.2, f_si_xc = 0.1, nW = nW, nXin = nXin, other_cat_an = 0, steady = false) annotation(
...
//
  ...
  model Ari_SS_vec_
    ...
    //Equazioni specificate nel modello di livello in cui attacchi la sorgente (CpombiTimeTable)
    //Puoi poi testare pezzi singoli anche intermedi attaccandoci a quel livello i tuoi input
    ADM1_P.BlockLibrary.Digester digester(S_gas_ch4(fixed = false), S_gas_co2(fixed = false), S_gas_h2(fixed = false), S_h(fixed = true, start = 7.17353e-8), V_gas = 0.077 - 0.051, V_liq = 0.051, X_aa(start = 0.61), X_ac(start = 0.44), X_c(start = 0.1), X_c4(start = 0.23), X_fa(start = 0.17), X_h2(start = 0.21), X_pro(start = 0.07), X_su(start = 0.17), f_ch_xc = 0.2, f_li_xc = 0.3, f_pr_xc = 0.2, f_si_xc = 0, ideality = true, k_dis = 1.2, k_hyd_xchm = 0.2, k_hyd_xli = 0.2, k_hyd_xprs = 0.15, kla = 80, multihyd = false, nW = nW, noXc = false, open = false, p_gas_ch4(fixed = true, start = 0.65), p_gas_co2(fixed = true), p_gas_h2(fixed = true), precipitation = false) annotation(
      Placement(visible = true, transformation(origin = {42, 14}, extent = {{-18, -18}, {18, 18}}, rotation = 0)));
    BlockLibrary.Probe_P Probe_P annotation(
      Placement(visible = true, transformation(origin = {-56, 72}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ADM1_P.BlockLibrary.Gas_Meter_stream Gas_Meter annotation(
      Placement(visible = true, transformation(origin = {58, 52}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Continuous.LimPID PID_Pgas(Td = 0.0, Ti = 0.5, controllerType = Modelica.Blocks.Types.SimpleController.P, initType = Modelica.Blocks.Types.InitPID.SteadyState, k = 10, limitsAtInit = true, strict = false, yMax = 10000, yMin = 0) annotation(
      Placement(visible = true, transformation(origin = {-6, 72}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.TimeTable timeTable1(table = [0, 310.15; 732, 310.15], timeScale = 86400) annotation(
      Placement(visible = true, transformation(origin = {-42, -78}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    BlockLibrary.Sink_vec sink_vec(nW = nW) annotation(
      Placement(visible = true, transformation(origin = {86, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ADM1_P.BlockLibrary.Feed_Source_Ari_vec_ feed_Source_Ari(Carbonates_in = 0, Catenacci = true, Flores = false, ID = 1, S_an_in = 0.04, TKN = true, f_ch_xc = 0.2, f_li_xc = 0.3, f_pr_xc = 0.2, f_si_xc = 0, nW = nW, nXin = nXin, noXc = false, other_cat_an = 0, steady = false) annotation(
      Placement(visible = true, transformation(origin = {-60, -20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
...
//
    ...
    connect(combiTimeTable1.y[1], feed_Source_Ari.inlet[1]) annotation(
      Line(points = {{-104, -40}, {-68, -40}, {-68, -24}}, color = {0, 0, 127}, thickness = 0.5));
//connect(Input_CattleSlurry.y, feed_Source_Ari.inlet[2:end]) annotation(
//  Line(points = {{-102, -20}, {-68, -20}, {-68, -24}}, color = {0, 0, 127}, thickness = 0.5));
    connect(combiTimeTable2.y[1], feed_Source_Ari.inlet[18]) annotation(
      Line(points = {{-134, -28}, {-68, -28}, {-68, -24}}, color = {0, 0, 127}, thickness = 0.5));
    annotation(
      experiment(StartTime = 0, StopTime = 8.4672e+06, Tolerance = 1e-09, Interval = 66.9455),
      __OpenModelica_commandLineOptions = "--matchingAlgorithm=PFPlusExt --indexReductionMethod=dynamicStateSelection -d=initialization,NLSanalyticJacobian",
      __OpenModelica_simulationFlags(lv = "LOG_STATS", s = "dassl"));
  end Ari_SS_vec_;

  model BSM2_stream_vec
    ...
    BlockLibrary.Sink_vec sink_vec(nW = nW) annotation(
      Placement(visible = true, transformation(origin = {86, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ADM1_P.BlockLibrary.Feed_Source_Ari_vec_ feed_Source_Ari(Carbonates_in = 384, Catenacci = false, Flores = true, ID = 1, S_an_in = 0, TKN = true, f_ch_xc = 0.2, f_li_xc = 0.3, f_pr_xc = 0.2, f_si_xc = 0, nW = nW, nXin = nXin, noXc = true, steady = false) annotation(
      Placement(visible = true, transformation(origin = {-60, -20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
...
//
  ...
  model SAntonio
    ...
    ADM1_P.BlockLibrary.Feed_Source_Ari_vec_ feed_Source_Ari(BD_ch = 83, BD_li = 100, BD_pr = 78, Carbonates_in = 0, Catenacci = false, Flores = false, ID = 3, TKN = true, f_ch_xc = 0.2, f_li_xc = 0.3, f_pr_xc = 0.2, f_si_xc = 0, nW = nW, nXin = nXin, noXc = true, other_cat_an = 0, steady = false, weender = true) annotation(
    ...
    ADM1_P.BlockLibrary.Feed_Source_Ari_vec_ feed_Source_Ari_vec_(BD_ch = 31, BD_li = 50, BD_pr = 73, Catenacci = false, Flores = false, ID = 2, TKN = true, f_ch_xc = 0.2, f_li_xc = 0.3, f_pr_xc = 0.2, f_si_xc = 0, nW = nW, nXin = nXin, noXc = true, other_cat_an = 0, steady = false, weender = false) annotation(
    ...
    ADM1_P.BlockLibrary.Feed_Source_Ari_vec_ feed_Source_Ari_vec_1(BD_ch = 43, BD_li = 50, BD_pr = 75, Carbonates_in = 0, Catenacci = false, Flores = false, ID = 1, TKN = true, f_ch_xc = 0.2, f_li_xc = 0.3, f_pr_xc = 0.2, f_si_xc = 0, nW = nW, nXin = nXin, noXc = true, other_cat_an = 0, steady = false, weender = true) annotation(
