package ADM1_P
  record ParametersADM1_operational
    ...
    //2 parallel fermenters (H=6 m, D=18m, Vtot=1527 m3)
    //1 post-fermenter (H=6 m, D=30m, Vtot=4241 m3)
    parameter Modelica.SIunits.Volume V_liq = 1527*2*0.8 "7430*2; Single Digester usefull working volume [m3]";
    parameter Modelica.SIunits.Volume V_liq2 = 4241*0.1 "Post-fermenter usefull working volume [m3]";
    parameter Modelica.SIunits.Volume V_gas = 1527*2*(1-0.8) "0.10884; Single Digester headspace as A*H-usefull working volume [m3]";
    parameter Modelica.SIunits.Volume V_gas2 = 4241*(1-0.9) "Post-fermenter headspace [m3]";
    ...
    parameter Types.Pressure_bar P_atm = 1.013 "Pressure of atmosphere [bar]";
...
<p> Last review: <em> December 06, 2020 </em></p></html>"));
  ...
  record StateVariablesADM1
    ...
    Types.MolarDensity_M S_h(fixed = true, start = 1.51014e-08, nominal = 1e-8) "8/10";
...
//Inorganic Phosforous Balance
  ...
  ADM1_P.Interfaces.Digestate Digestate_In annotation(
        Placement(visible = true, transformation(origin = {-74, 16}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-76, -38}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealInput Fractions_In[9] annotation(
        Placement(visible = true, transformation(origin = {-72, -88}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {-72, -16}, extent = {{-8, -8}, {8, 8}}, rotation = 0)));
    equation
      f_ch1 = Fractions_In[1];
      f_ch2 = Fractions_In[2];
      f_ch3 = Fractions_In[3];
...
//I = 0.5 * (S_ca * z_ca ^ 2 + S_po4 * z_po4 ^ 2 + S_nh4 * z_nh4 ^ 2 + S_mg * z_mg ^ 2 + S_hpo4 * 2 ^ 2 + S_h2po4 * 1 ^ 2 + S_hco3 * 1 ^ 2);
      ...
      rp1 = kcryst_acp * X_acp * ((a_ca ^ 3 * a_po4 ^ 2 / Ksp_acp) ^ (1 / 5) - 1) ^ 2 * SI_acp;
      rp2 = kcryst_stru * X_stru * ((a_nh4 * a_po4 * a_mg / Ksp_stru) ^ (1 / 3) - 1) ^ 2 * SI_stru;
      24 * 3600 * der(X_acp) = Digestate_In.Q / V_liq2 * (-X_acp) + rp1;
      24 * 3600 * der(X_stru) = Digestate_In.Q / V_liq2 * (-X_stru) + rp2;
//
//TEMPERATURE DEPENDENT PARAMETERS
//Acidity constants with temperature correction
      Ka_co2 = 10 ^ (-pKa_co2) * exp(deltaH0_Ka_co2 / (R * 100) * (1 / T_ref - 1 / T_op)) * convKa "CO2 acidity constant with temperature correction - unit = M";
...
//
//WATER PHASE EQUATIONS
      Digestate_In.Q + Digestate_Out.Q = 0;
//
//Particulate matter
      24 * 3600 * der(X_c) = Digestate_In.Q / V_liq2 * (Digestate_In.X_c - X_c) - r1 "+ r13 + r14 + r15 + r16 + r17 + r18 + r19";
      24 * 3600 * der(X_ch) = Digestate_In.Q / V_liq2 * (Digestate_In.X_ch - X_ch) - r2_1 - r2_2 - r2_3 + f_ch_xb * (r13 + r14 + r15 + r16 + r17 + r18 + r19);
      24 * 3600 * der(X_pr) = Digestate_In.Q / V_liq2 * (Digestate_In.X_pr - X_pr) - r3_1 - r3_2 - r3_2 + f_pr_xb * (r13 + r14 + r15 + r16 + r17 + r18 + r19);
      24 * 3600 * der(X_li) = Digestate_In.Q / V_liq2 * (Digestate_In.X_li - X_li) - r4_1 - r4_2 - r4_3 + f_li_xb * (r13 + r14 + r15 + r16 + r17 + r18 + r19);
      24 * 3600 * der(X_i) = Digestate_In.Q / V_liq2 * (Digestate_In.X_i - X_i) + f_xi_xb * (r13 + r14 + r15 + r16 + r17 + r18 + r19);
//
//Soluble matter
      24 * 3600 * der(S_i) = Digestate_In.Q / V_liq2 * (Digestate_In.S_i - S_i) + f_si_xb * (r13 + r14 + r15 + r16 + r17 + r18 + r19);
      24 * 3600 * der(S_su) = Digestate_In.Q / V_liq2 * (Digestate_In.S_su - S_su) + r2_1 + r2_2 + r2_3 + (1 - f_fa_li) * (r4_1 + r4_2 + r4_3) - r5;
      24 * 3600 * der(S_aa) = Digestate_In.Q / V_liq2 * (Digestate_In.S_aa - S_aa) + r3_1 + r3_2 + r3_3 - r6;
      24 * 3600 * der(S_fa) = Digestate_In.Q / V_liq2 * (Digestate_In.S_fa - S_fa) + f_fa_li * (r4_1 + r4_2 + r4_3) - r7;
      24 * 3600 * der(S_va) = Digestate_In.Q / V_liq2 * (Digestate_In.S_va - S_va) + (1 - Y_aa) * f_va_aa * r6 - r8;
      24 * 3600 * der(S_bu) = Digestate_In.Q / V_liq2 * (Digestate_In.S_bu - S_bu) + (1 - Y_su) * f_bu_su * r5 + (1 - Y_aa) * f_bu_aa * r6 - r9;
      24 * 3600 * der(S_pro) = Digestate_In.Q / V_liq2 * (Digestate_In.S_pro - S_pro) + (1 - Y_su) * f_pro_su * r5 + (1 - Y_aa) * f_pro_aa * r6 + (1 - Y_c4) * 0.54 * r8 - r10;
      24 * 3600 * der(S_ac) = Digestate_In.Q / V_liq2 * (Digestate_In.S_ac - S_ac) + (1 - Y_su) * f_ac_su * r5 + (1 - Y_aa) * f_ac_aa * r6 + (1 - Y_fa) * 0.7 * r7 + (1 - Y_c4) * 0.31 * r8 + (1 - Y_c4) * 0.8 * r9 + (1 - Y_pro) * 0.57 * r10 - r11;
      24 * 3600 * der(S_h2) = Digestate_In.Q / V_liq2 * (Digestate_In.S_h2 - S_h2) + (1 - Y_su) * f_h2_su * r5 + (1 - Y_aa) * f_h2_aa * r6 + (1 - Y_fa) * 0.3 * r7 + (1 - Y_c4) * 0.15 * r8 + (1 - Y_c4) * 0.2 * r9 + (1 - Y_pro) * 0.43 * r10 - r12 - rt8;
      24 * 3600 * der(S_ch4) = Digestate_In.Q / V_liq2 * (Digestate_In.S_ch4 - S_ch4) + (1 - Y_ac) * r11 + (1 - Y_h2) * r12 - rt9;
      24 * 3600 * der(S_ic) = Digestate_In.Q / V_liq2 * (Digestate_In.S_ic - S_ic) - (v1 * r1 + v2 * (r2_1 + r2_2 + r2_3) + v3 * (r3_1 + r3_2 + r3_3) + v4 * (r4_1 + r4_2 + r4_3) + v5 * r5 + v6 * r6 + v7 * r7 + v8 * r8 + v9 * r9 + v10 * r10 + v11 * r11 + v12 * r12 + v13 * (r13 + r14 + r15 + r16 + r17 + r18 + r19)) - rt10;
      24 * 3600 * der(S_in) = Digestate_In.Q / V_liq2 * (Digestate_In.S_in - S_in) - Y_su * N_bac * r5 + (N_aa - Y_aa * N_bac) * r6 - Y_fa * N_bac * r7 - Y_c4 * N_bac * r8 - Y_c4 * N_bac * r9 - Y_pro * N_bac * r10 - Y_ac * N_bac * r11 - Y_h2 * N_bac * r12 + (N_bac - N_i * f_xi_xb - N_aa * f_pr_xb) * (r13 + r14 + r15 + r16 + r17 + r18 + r19) - rp2;
      24 * 3600 * der(S_ip) = Digestate_In.Q / V_liq2 * (Digestate_In.S_ip - S_ip) + P_li * (r4_1 + r4_2 + r4_3) - Y_su * P_bac * r5 - Y_aa * P_bac * r6 - Y_fa * P_bac * r7 - Y_c4 * P_bac * r8 - Y_c4 * P_bac * r9 - Y_pro * P_bac * r10 - Y_ac * P_bac * r11 - Y_h2 * P_bac * r12 + (P_bac - P_i * f_xi_xb - P_li * f_li_xb) * (r13 + r14 + r15 + r16 + r17 + r18 + r19) - 2 * rp1 - rp2;
//
//Biomasses
      24 * 3600 * der(X_su) = Digestate_In.Q / V_liq2 * (Digestate_In.X_su - X_su) + Y_su * r5 - r13;
      24 * 3600 * der(X_aa) = Digestate_In.Q / V_liq2 * (Digestate_In.X_aa - X_aa) + Y_aa * r6 - r14;
      24 * 3600 * der(X_fa) = Digestate_In.Q / V_liq2 * (Digestate_In.X_fa - X_fa) + Y_fa * r7 - r15;
      24 * 3600 * der(X_c4) = Digestate_In.Q / V_liq2 * (Digestate_In.X_c4 - X_c4) + Y_c4 * r8 + Y_c4 * r9 - r16;
      24 * 3600 * der(X_pro) = Digestate_In.Q / V_liq2 * (Digestate_In.X_pro - X_pro) + Y_pro * r10 - r17;
      24 * 3600 * der(X_ac) = Digestate_In.Q / V_liq2 * (Digestate_In.X_ac - X_ac) + Y_ac * r11 - r18;
      24 * 3600 * der(X_h2) = Digestate_In.Q / V_liq2 * (Digestate_In.X_h2 - X_h2) + Y_h2 * r12 - r19;
//
//Cations and anions
      24 * 3600 * der(S_cat) = Digestate_In.Q / V_liq2 * (Digestate_In.S_cat - S_cat);
      24 * 3600 * der(S_ca) = Digestate_In.Q / V_liq2 * (Digestate_In.S_ca - S_ca) - 3 * rp1;
      24 * 3600 * der(S_mg) = Digestate_In.Q / V_liq2 * (Digestate_In.S_mg - S_mg) - rp2;
      24 * 3600 * der(S_an) = Digestate_In.Q / V_liq2 * (Digestate_In.S_an - S_an);
      24 * 3600 * der(S_vam) = -ra4;
      24 * 3600 * der(S_bum) = -ra5;
      24 * 3600 * der(S_prom) = -ra6;
      24 * 3600 * der(S_acm) = -ra7;
...
//
//DIGESTER'S OPERATIVE PARAMETERS
      HRT = V_liq2 / Digestate_In.Q "Hyraulic Retention Time [d]";
...
//COD Balance
      COD_S = S_aa + S_ac + S_bu + S_ch4 + S_fa + S_h2 + S_i + S_pro + S_su + S_va "Soluble COD concentration";
      COD_X = X_aa + X_ac + X_c4 + X_ch + X_fa + X_h2 + X_i + X_li + X_pr + X_pro + X_su "+X_c; Particulate COD concentration";
      COD_balance_in = Digestate_In.Q * (Digestate_In.S_aa + Digestate_In.S_ac + Digestate_In.S_bu + Digestate_In.S_ch4 + Digestate_In.S_fa + Digestate_In.S_h2 + Digestate_In.S_i + Digestate_In.S_pro + Digestate_In.S_su + Digestate_In.S_va + Digestate_In.X_aa + Digestate_In.X_ac + Digestate_In.X_c4 + Digestate_In.X_fa + Digestate_In.X_h2 + Digestate_In.X_pro + Digestate_In.X_su + Digestate_In.X_ch + Digestate_In.X_c + Digestate_In.X_i + Digestate_In.X_li + Digestate_In.X_pr);
      COD_balance_out = Digestate_Out.Q * (COD_S + COD_X);
      COD_balance_gas = Biogas_Out.Q_gas_N * (p_gas_ch4 / (P_gas - p_gas_h2o) / (R / 64 * 273.15 / 1.01325) + p_gas_h2 / (P_gas - p_gas_h2o) / (R / 16 * 273.15 / 1.01325));
      24 * 3600 * der(CH4_cum) = ((1 - Y_ac) * r11 + (1 - Y_h2) * r12) * V_liq2;
      24 * 3600 * der(V_in_cum) = Digestate_In.Q;
      24 * 3600 * der(COD_balance_in_cum) = COD_balance_in;
      24 * 3600 * der(COD_balance_out_cum) = COD_balance_out;
      24 * 3600 * der(COD_balance_gas_cum) = COD_balance_gas;
//Inorganic Carbon Balance
      IC_balance_in = Digestate_In.Q * (Digestate_In.S_ic + C_xc * Digestate_In.X_c + C_si * Digestate_In.S_i + C_xi * Digestate_In.X_i + C_ch * Digestate_In.X_ch + C_pr * Digestate_In.X_pr + C_li * Digestate_In.X_li + C_su * Digestate_In.S_su + C_aa * Digestate_In.S_aa + C_fa * Digestate_In.S_fa + C_va * Digestate_In.S_va + C_bu * Digestate_In.S_bu + C_pro * Digestate_In.S_pro + C_ac * Digestate_In.S_ac + C_ch4 * Digestate_In.S_ch4 + C_bac * (Digestate_In.X_su + Digestate_In.X_aa + Digestate_In.X_fa + Digestate_In.X_c4 + Digestate_In.X_pro + Digestate_In.X_ac + Digestate_In.X_h2));
      IC_balance_out = Digestate_Out.Q * (S_ic + C_si * S_i + C_xi * X_i + C_ch * X_ch + C_pr * X_pr + C_li * X_li + C_su * S_su + C_aa * S_aa + C_fa * S_fa + C_va * S_va + C_bu * S_bu + C_pro * S_pro + C_ac * S_ac + C_ch4 * S_ch4 + C_bac * (X_su + X_aa + X_fa + X_c4 + X_pro + X_ac + X_h2)) "+C_xc * X_c";
      IC_balance_gas = Biogas_Out.Q_gas_N * (C_ch4 * p_gas_ch4 / (P_gas - p_gas_h2o) / (R / 64 * 273.15 / 1.01325) + p_gas_co2 / (P_gas - p_gas_h2o) / R / 273.15 * 1.01325);
      24 * 3600 * der(IC_balance_in_cum) = IC_balance_in;
      24 * 3600 * der(IC_balance_out_cum) = IC_balance_out;
      24 * 3600 * der(IC_balance_gas_cum) = IC_balance_gas;
//Inorganic Nitrogen Balance
      IN_balance_in = Digestate_In.Q * (Digestate_In.S_in + N_i * (Digestate_In.S_i + Digestate_In.X_i) + N_aa * (Digestate_In.S_aa + Digestate_In.X_pr) + N_bac * (Digestate_In.X_su + Digestate_In.X_aa + Digestate_In.X_fa + Digestate_In.X_c4 + Digestate_In.X_pro + Digestate_In.X_ac + Digestate_In.X_h2));
      IN_balance_out = Digestate_Out.Q * (S_in + N_i * (S_i + X_i) + N_aa * (S_aa + X_pr) + N_bac * (X_su + X_aa + X_fa + X_c4 + X_pro + X_ac + X_h2) + X_stru);
      24 * 3600 * der(IN_balance_in_cum) = IN_balance_in;
      24 * 3600 * der(IN_balance_out_cum) = IN_balance_out;
//Inorganic Phosforous Balance
      IP_balance_in = Digestate_In.Q * (Digestate_In.S_ip + P_i * (Digestate_In.S_i + Digestate_In.X_i) + P_li * Digestate_In.X_li + P_bac * (Digestate_In.X_su + Digestate_In.X_aa + Digestate_In.X_fa + Digestate_In.X_c4 + Digestate_In.X_pro + Digestate_In.X_ac + Digestate_In.X_h2));
      IP_balance_out = Digestate_Out.Q * (S_ip + P_i * (S_i + X_i) + P_li * X_li + P_bac * (X_su + X_aa + X_fa + X_c4 + X_pro + X_ac + X_h2) + 2 * X_acp + X_stru);
      24 * 3600 * der(IP_balance_in_cum) = IP_balance_in;
      24 * 3600 * der(IP_balance_out_cum) = IP_balance_out;
      24 * 3600 * der(Vcum) = Digestate_In.Q;
      annotation(
        Diagram,
        __OpenModelica_commandLineOptions = "--matchingAlgorithm=PFPlusExt --indexReductionMethod=dynamicStateSelection -d=initialization,NLSanalyticJacobian",
        __OpenModelica_simulationFlags(lv = "LOG_STATS", s = "dassl"),
...
//Alk = (Digestate_In.S_hco3 + Digestate_In.S_oh + Digestate_In.S_acm / Ac_conv + Digestate_In.S_prom / Pro_conv + Digestate_In.S_bum / Bu_conv + Digestate_In.S_vam / Va_conv) * 50 * 1000 "Alkalinity as mgCaCO3/L";
  ...
  class SynopticModel
    ADM1_P.BlockLibrary.Feed_Source_1 CattleManure annotation(
      Placement(visible = true, transformation(origin = {-176, 36}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    ADM1_P.BlockLibrary.Feeding_Tank feeding_Tank annotation(
      Placement(visible = true, transformation(origin = {-116, -2}, extent = {{-22, -22}, {22, 22}}, rotation = 0)));
    ADM1_P.BlockLibrary.Digester digester annotation(
      Placement(visible = true, transformation(origin = {-43, -5}, extent = {{-25, -25}, {25, 25}}, rotation = 0)));
  ...
  ADM1_P.BlockLibrary.Postfermenter postfermenter annotation(
      Placement(visible = true, transformation(origin = {93, 5}, extent = {{-25, -25}, {25, 25}}, rotation = 0)));
  ADM1_P.BlockLibrary.Sludge_Discharge sludge_Discharge annotation(
      Placement(visible = true, transformation(origin = {157, -17}, extent = {{-23, -23}, {23, 23}}, rotation = 0)));
  ADM1_P.BlockLibrary.Probe_T probe_T annotation(
  ...
  ADM1_P.BlockLibrary.Probe_CO2_perc probe_CO2_perc1 annotation(
      Placement(visible = true, transformation(origin = {150, 62}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
  connect(digester.Biogas_Out, Gas_Meter.Biogas_In) annotation(
      Line(points = {{-31.5, 14}, {-31.5, 36}, {-4, 36}}));
  connect(digester.T_op, Probe_T.y_T) annotation(
      Line(points = {{-61.5, -20}, {-69, -20}, {-69, -38}}, color = {0, 0, 127}));
  connect(Gas_Meter.CH4_perc_exp, probe_CH4_perc.y_CH4) annotation(
      Line(points = {{1.3, 43.9}, {-10.7, 43.9}, {-10.7, 51.9}}, color = {0, 0, 127}));
  connect(Gas_Meter.CO2_perc_exp, probe_CO2_perc.y_CO2) annotation(
      Line(points = {{6.7, 43.9}, {20.7, 43.9}, {20.7, 50.9}}, color = {0, 0, 127}));
  connect(PID_Pgas.y, digester.u) annotation(
      Line(points = {{-45, 52}, {-45, 35.25}, {-43, 35.25}, {-43, 18.5}}, color = {0, 0, 127}));
  connect(Probe_P.y_P, PID_Pgas.u_m) annotation(
      Line(points = {{-97, 52}, {-93, 52}, {-93, 30}, {-57, 30}, {-57, 40}}, color = {0, 0, 127}));
  connect(PID_Pgas.u_s, digester.P_gas) annotation(
      Line(points = {{-68, 52}, {-78, 52}, {-78, 32}, {-46, 32}, {-46, 18.5}}, color = {0, 0, 127}));
  connect(CattleSlurry.Slurry_Out, feeding_Tank.Slurry_In) annotation(
      Line(points = {{-157.2, -1}, {-144.2, -1}, {-144.2, 1}, {-131.2, 1}}));
  connect(CattleManure.Manure_Out, feeding_Tank.Manure_In) annotation(
      Line(points = {{-167.2, 29}, {-131.2, 29}, {-131.2, 11}}));
  connect(feeding_Tank.FeedMix_Out, digester.FeedMix_In) annotation(
      Line(points = {{-99.72, -0.68}, {-61.72, -0.68}, {-61.72, 6.32}}));
  connect(feeding_Tank.Fractions_Out, digester.Fractions_In) annotation(
      Line(points = {{-99.72, -5.08}, {-79.72, -5.08}, {-79.72, -14.08}, {-61.72, -14.08}}, color = {0, 0, 127}, thickness = 0.5));
  connect(digester.Digestate_Out, postfermenter.Digestate_In) annotation(
      Line(points = {{-24, -21.5}, {-8, -21.5}, {-8, -4.5}, {74, -4.5}}));
  connect(postfermenter.Digestate_Out, sludge_Discharge.Digestate_In) annotation(
      Line(points = {{112, -11.5}, {136, -11.5}}));
  connect(feeding_Tank.Fractions_Out, postfermenter.Fractions_In) annotation(
      Line(points = {{-99.72, -5.08}, {-47.72, -5.08}, {-47.72, 0.92}, {75.28, 0.92}}, color = {0, 0, 127}, thickness = 0.5));
  connect(limPID.y, postfermenter.u) annotation(
      Line(points = {{79, 50}, {93, 50}, {93, 28}}, color = {0, 0, 127}));
  connect(postfermenter.P_gas, limPID.u_s) annotation(
      Line(points = {{90, 28.5}, {56, 28.5}, {56, 50.5}}, color = {0, 0, 127}));
  connect(probe_P.y_P, limPID.u_m) annotation(
      Line(points = {{47, 24}, {69, 24}, {69, 38}, {67, 38}}, color = {0, 0, 127}));
  connect(postfermenter.Biogas_Out, gas_Meter.Biogas_In) annotation(
      Line(points = {{104.5, 24}, {124.5, 24}, {124.5, 36}}));
  connect(probe_CH4_perc1.y_CH4, gas_Meter.CH4_perc_exp) annotation(
      Line(points = {{120, 53}, {130, 53}, {130, 43}}, color = {0, 0, 127}));
  connect(probe_CO2_perc1.y_CO2, gas_Meter.CO2_perc_exp) annotation(
      Line(points = {{150, 53}, {134, 53}, {134, 43}}, color = {0, 0, 127}));
  connect(MaizeSilage.MaizeSilage_Out, feeding_Tank.MaizeSilage_In) annotation(
      Line(points = {{-161.2, -33}, {-131.2, -33}, {-131.2, -15}}));
  connect(probe_T.y_T, postfermenter.T_op) annotation(
      Line(points = {{70, -36}, {74, -36}, {74, -10}}, color = {0, 0, 127}));
  protected
    annotation(
