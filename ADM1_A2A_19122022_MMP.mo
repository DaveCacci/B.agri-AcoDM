package ADM1
  record ParametersADM1_operational
    parameter Real V_liq(unit = "m3") =  3350*4 "Single Digester usefull working volume";
    parameter Real V_gas(unit = "m3") = (26^2*7*3.14/4)*4-V_liq "0.10884; Single Digester headspace as A*H-usefull working volume";
  ...
  record StateVariablesADM1
    ...
    Real S_ip(fixed = true, start = 0.01) "8/10";
    Real S_cat(fixed = true, start = 0.0708801468406167);
    Real S_ca(fixed = true, start = 0.001);
    Real S_mg(fixed = true, start = 0.001);
    ...
    Real S_hpo4(fixed = false, start = 0.00444*0.94) "8/10";
    Real S_h2po4(fixed = true, start = 0.00556*0.94) "8/10";
    Real S_po4(fixed = true, start = 0.00056) "8/10";
...
//Verify balance
  ...
  Modelica.Blocks.Interfaces.RealInput u annotation(
      ...
      Real X_acp(fixed=true, start=1e-6) "mol/L";
      Real X_stru(fixed=true, start=1e-6) "mol/L";
      Real a_ca;
      Real a_po4;
      Real a_nh4;
      Real a_mg;
      ...
      parameter Real Ksp_acp(unit="conc") = 10^(-28.92);
      parameter Real kcryst_acp(unit="d-1") = 3*(24*60);
      parameter Real Ksp_stru(unit="conc") = 10^(-13.6);
...
//Cations and anions
      ...
      Theta = S_cat + S_nh4 + 2*S_ca + 2*S_mg - S_hco3 - S_acm / Ac_conv - S_prom / Pro_conv - S_bum / Bu_conv - S_vam / Va_conv - S_an - 2*S_hpo4 - S_h2po4 - 3*S_po4 "manca S_ca?";
      S_nh4 = S_in - S_nh3;
      S_co2 = S_ic - S_hco3;
...
//Inorganic Nitrogen Balance
      ...
      IN_balance_out = Digestate_Out.Q * (S_in + N_i * (S_i + X_i) + N_aa * (S_aa + X_pr) + N_bac * (X_su + X_aa + X_fa + X_c4 + X_pro + X_ac + X_h2));
...
//Alk = (Digestate_In.S_hco3 + Digestate_In.S_oh + Digestate_In.S_acm / Ac_conv + Digestate_In.S_prom / Pro_conv + Digestate_In.S_bum / Bu_conv + Digestate_In.S_vam / Va_conv) * 50 * 1000 "Alkalinity as mgCaCO3/L";
  ...
  protected
    annotation(
      ...
      __OpenModelica_commandLineOptions = "--matchingAlgorithm=PFPlusExt --indexReductionMethod=dynamicStateSelection -d=initialization,NLSanalyticJacobian",
      experiment(StartTime = 0, StopTime = 17.28e+06, Tolerance = 1e-06, Interval = 240),
  Diagram);
  end SynopticModel;
  annotation(
