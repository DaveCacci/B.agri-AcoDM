...
//Outputs
    ...
    model PostDigester_
    extends Digester(redeclare connector FeedMix_In=ADM1_P.Interfaces.Digestate);
    //ADM1_P.Interfaces.Digestate FeedMix_In annotation(
      //    Placement(visible = true, transformation(origin = {-74, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-76, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    equation
    ...
    model Digester_
    extends Digester(redeclare connector FeedMix_In=ADM1_P.Interfaces.Feed);
    //ADM1_P.Interfaces.Feed FeedMix_In annotation(
      //    Placement(visible = true, transformation(origin = {-54, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-76, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    equation
  ...
  ADM1_P.Interfaces.Feed_stream outlet2(X=X) annotation(
        Placement(visible = true, transformation(origin = {94, -32}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {92, -52}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    equation
    //Acting as a mixer
      ...
      outlet2.X_h2 = inStream(inlet.X_h2);
      outlet1.X_i = inStream(inlet.X_i);
      outlet2.X_i = inStream(inlet.X_i);
    ...
    equation
    //Block variables
      Feed_Out.Q = -inflow[1] "m3/d";
      Feed_Out.X_h2[1] = inflow[2] "gCOD/L";
      Feed_Out.X_i = inflow[3];
      annotation(
        __OpenModelica_simulationFlags(lv = "LOG_STATS", s = "dassl"),
        experiment(StartTime = 0, StopTime = 1, Tolerance = 1e-6, Interval = 0.002),
        __OpenModelica_commandLineOptions = "--matchingAlgorithm=PFPlusExt --indexReductionMethod=dynamicStateSelection -d=initialization,NLSanalyticJacobian");
...
//CattleManure
  ...
  connect(Input_T, probe_T_.Input_T) annotation(
...
//CattleManure
  ...
  model Try_stream
  Modelica.Blocks.Sources.CombiTimeTable Feed1(columns = integer({2, 3, 4}), extrapolation = Modelica.Blocks.Types.Extrapolation.NoExtrapolation, fileName = "C:/Users/lenovo/Desktop/KINGSTON/DOTTORATO/Modelica/Pilot_plant/Stream_try_feed1.txt", smoothness = Modelica.Blocks.Types.Smoothness.ConstantSegments, startTime = 0, table = [0, 0; 1, 1; 2, 4], tableName = "Feed1", tableOnFile = true, timeEvents = Modelica.Blocks.Types.TimeEvents.Always, timeScale = 86400, verboseExtrapolation = true, verboseRead = true) annotation(
      Placement(visible = true, transformation(origin = {-104, 32}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  BlockLibrary.Feed_Source_stream feed1 annotation(
  ...
  BlockLibrary.Feed_Source_stream feed2 annotation(
      Placement(visible = true, transformation(origin = {-38, -12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Sources.CombiTimeTable Feed2(columns = integer({2, 3, 4}), extrapolation = Modelica.Blocks.Types.Extrapolation.NoExtrapolation, fileName = "C:/Users/lenovo/Desktop/KINGSTON/DOTTORATO/Modelica/Pilot_plant/Stream_try_feed2.txt", smoothness = Modelica.Blocks.Types.Smoothness.ConstantSegments, startTime = 0, table = [0, 0; 1, 1; 2, 4], tableName = "Feed2", tableOnFile = true, timeEvents = Modelica.Blocks.Types.TimeEvents.Always, timeScale = 86400, verboseExtrapolation = true, verboseRead = true) annotation(
      Placement(visible = true, transformation(origin = {-102, -24}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ADM1_P.BlockLibrary.Sink sink annotation(
      Placement(visible = true, transformation(origin = {44, -28}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ADM1_P.BlockLibrary.Sink sink1 annotation(
  ...
  //  Feed2.y[3] = feed2.Xh2_in;
    connect(Feed1.y, feed1.inflow) annotation(
      Line(points = {{-92, 32}, {-76, 32}, {-76, 30}}, color = {0, 0, 127}, thickness = 0.5));
    connect(feed1.Feed_Out, feeding_Tank_stream.inlet) annotation(
      Line(points = {{-60, 28}, {-10, 28}, {-10, 2}}));
    connect(feed2.Feed_Out, feeding_Tank_stream.inlet) annotation(
      Line(points = {{-30, -18}, {-10, -18}, {-10, 2}}));
    connect(Feed2.y, feed2.inflow) annotation(
      Line(points = {{-90, -24}, {-75.5, -24}, {-75.5, -16}, {-45, -16}}, color = {0, 0, 127}, thickness = 0.5));
  connect(feeding_Tank_stream.outlet1, sink1.inlet) annotation(
      Line(points = {{10, 2}, {47, 2}, {47, 25}}));
  connect(feeding_Tank_stream.outlet2, sink.inlet) annotation(
      Line(points = {{10, -4}, {30, -4}, {30, -35}, {53, -35}}));
  end Try_stream;
