...
//Gas components
  ...
  record ParametersADM1_operational
    parameter Modelica.SIunits.Volume V_liq = 3400 "SRT=19 o 20. Portata 190 m3/d. Single Digester usefull working volume";
    parameter Modelica.SIunits.Volume V_liq2 = 743 "Post-fermenter usefull working volume [m3]";
    parameter Modelica.SIunits.Volume V_gas = 100 "0.10884; Single Digester headspace as A*H-usefull working volume";
    ...
    parameter Modelica.SIunits.Temperature T_op_mean = 35+273.15 "Mean operative temperature";
...
<p> Last review: <em> December 06, 2020 </em></p></html>"));
  ...
  record StateVariablesADM1
    ...
    Modelica.SIunits.MassConcentration S_h2(fixed = true, start = 4.11532e-07) "Soluble hydrogen concentration in digestate [kgCOD/m3]";
    Modelica.SIunits.MassConcentration S_ch4(fixed = true, start = 0.0474429) "Soluble methane concentration in digestate [kgCOD/m3]";
    Types.MolarDensity_M S_ic(fixed = true, start = 0.312386) "Soluble inorganic carbon concentration in digestate [kmol/m3]";
    Types.MolarDensity_M S_in(fixed = true, start = 0.242044) "Soluble inorganic nitrogen concentration in digestate [kmol/m3]";
    Types.MolarDensity_M S_ip(fixed = true, start = 0.00636595,nominal = 1e-3) "8/10";
    Types.MolarDensity_M S_cat(fixed = true, start = 0.118805) "Soluble cationic ion concentration in digestate [kmol/m3]";
    Types.MolarDensity_M S_ca(fixed = false, start = 0.00027486,nominal = 1e-4);
    Types.MolarDensity_M S_mg(fixed = true, start = 0.000128337,nominal = 1e-5);
    Types.MolarDensity_M S_an(fixed = false, start = 0.0200409);
    Types.MolarDensity_M S_hco3(fixed = false, start = 0.302539) "8/10";
    Types.MolarDensity_M S_co3(fixed = false, start = 4.01e-3, nominal = 10-3) "8/10";
    Types.MolarDensity_M S_nh3(fixed = false, start = 0.242044-0.161871282,nominal = 1e-2) "8/10";
    Types.MolarDensity_M S_nh4(fixed = false, start = 0.161871282,nominal = 1e-1) "8/10";             //from I value fixed
    ...
    Types.MolarDensity_M S_h(fixed = true, start = 1.63798530040609E-08, nominal = 1e-8, min = 1e-8, max = 1.7e-8) "8/10";
...
//entra come totale o come ione? Analisi controioni? Trovo S_cat togliendo i cationi noti.
    ...
    //not used here
      ...
      Ca_in2 = 0.007 " 0.007 mol/L";
      Mg_in2 = 0.013" 0.013 mol/L";
    //Computation of soluble gas
    ...
    //Slurry_Out.S_ic = S_hco3_in2 * (S_h_in2 + Ka_co2_in2) / Ka_co2_in2 "mol/L";
      ...
      Slurry_Out.S_ic = 384 / 12.011 / 1000 "mol/L";
      Slurry_Out.S_in = Ammonium_in2 / 14.0067 / 1000 "mol/L";
      Slurry_Out.S_ip = Fosfates_in2 / 30.973762 / 1000 "mol/L";
      S_nh4_in2 = Slurry_Out.S_in * S_h_in2 / (Ka_nh4_in2 + S_h_in2) "mol/L";
    //Slurry_Out.S_an = S_an_input2 "mol/L";
      Slurry_Out.S_an = 0.0000 "mol/L";
...
//
//I calculation
      I = if noEvent(0.5 * (S_ca * z_ca ^ 2 + S_po4 * z_po4 ^ 2 + S_nh4 * z_nh4 ^ 2 + S_mg * z_mg ^ 2 + S_hpo4 * 2 ^ 2 + S_h2po4 * 1 ^ 2 + S_hco3 * 1 ^ 2 + S_cat * 1 ^ 2 + S_an * 1 ^ 2 + S_co3 * 2^2 + 1^2*(S_acm / Ac_conv + S_prom / Pro_conv + S_bum / Bu_conv + S_vam / Va_conv))) < 0 then 0.2 else 0.5 * (S_ca * z_ca ^ 2 + S_po4 * z_po4 ^ 2 + S_nh4 * z_nh4 ^ 2 + S_mg * z_mg ^ 2 + S_hpo4 * 2 ^ 2 + S_h2po4 * 1 ^ 2 + S_hco3 * 1 ^ 2 + S_cat * 1 ^ 2 + S_an * 1 ^ 2 + S_co3 * 2^2 + 1^2*(S_acm / Ac_conv + S_prom / Pro_conv + S_bum / Bu_conv + S_vam / Va_conv));
//Activity coefficients calculation
      g_mono = 10 ^ (-0.5 * z_h ^ 2 * (I ^ 0.5 / (1 + I ^ 0.5) - 0.3 * I));
      g_bi = 10 ^ (-0.5 * z_ca ^ 2 * (I ^ 0.5 / (1 + I ^ 0.5) - 0.3 * I));
      g_tri = 10 ^ (-0.5 * z_po4 ^ 2 * (I ^ 0.5 / (1 + I ^ 0.5) - 0.3 * I));
...
//a_po4 = g_tri * S_po4;
      a_mg = g_bi * S_mg;
//a_nh4 = g_nh4 * S_nh4;
//a_nh4 = S_in * (a_h)^2 / ((a_h^3/g_nh4)+(Ka_nh4*a_h^2));
...
//MPP submodel
      Ksp_acp = 10 ^ (-pKsp_acp) * exp(deltaH0_Ksp_acp / (R * 100) * (1 / T_ref - 1 / T_op_mean));
      Ksp_stru = 10 ^ (-pKsp_stru) * exp(deltaH0_Ksp_stru / (R * 100) * (1 / T_ref - 1 / T_op_mean));
...
Theta = S_cat + S_nh4 + 2 * S_ca + 2 * S_mg - S_hco3 - S_acm / Ac_conv - S_prom / Pro_conv - S_bum / Bu_conv - S_vam / Va_conv - S_an - 2 * S_hpo4 - S_h2po4 - 3 * S_po4 - S_h2po4 - 2 * S_co3;
//ODE;
  //Theta = 0;
...
//I = 0.5 * (S_ca * z_ca ^ 2 + S_po4 * z_po4 ^ 2 + S_nh4 * z_nh4 ^ 2 + S_mg * z_mg ^ 2 + S_hpo4 * 2 ^ 2 + S_h2po4 * 1 ^ 2 + S_hco3 * 1 ^ 2);
      ...
      rp1 = kcryst_acp * X_acp * ((a_ca ^ 3 * a_po4 ^ 2 / Ksp_acp) ^ (1 / 5) - 1) ^ 2 * SI_acp;
      rp2 = kcryst_stru * X_stru * ((a_nh4 * a_po4 * a_mg / Ksp_stru) ^ (1 / 3) - 1) ^ 2 * SI_stru;
      24 * 3600 * der(X_acp) = Q / V_liq2 * (-X_acp) + rp1;
      24 * 3600 * der(X_stru) = Q / V_liq2 * (-X_stru) + rp2;
//
//TEMPERATURE DEPENDENT PARAMETERS
//Acidity constants with temperature correction
      Ka_co2 = 10 ^ (-pKa_co2) * exp(deltaH0_Ka_co2 / (R * 100) * (1 / T_ref - 1 / T_op)) * convKa "CO2 acidity constant with temperature correction - unit = M";
...
connect(MaizeSilage.MaizeSilage_Out, feeding_Tank.MaizeSilage_In) annotation(
  ...
  class SynopticModel2
    ADM1_P.BlockLibrary.Feed_Source_1 CattleManure annotation(
      Placement(visible = true, transformation(origin = {-128, 54}, extent = {{-12, -12}, {12, 12}}, rotation = 0)));
    ADM1_P.BlockLibrary.Feed_Source_2 CattleSlurry annotation(
      Placement(visible = true, transformation(origin = {-116, 26}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ADM1_P.BlockLibrary.Feed_Source_3 MaizeSilage annotation(
      Placement(visible = true, transformation(origin = {-120, -6}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  equation
    ...
    connect(CattleSlurry.Slurry_Out, feeding_Tank.Slurry_In) annotation(
      Line(points = {{-108, 20}, {-82, 20}}));
    connect(CattleManure.Manure_Out, feeding_Tank.Manure_In) annotation(
      Line(points = {{-117, 46}, {-100.5, 46}, {-100.5, 30}, {-82, 30}}));
    connect(MaizeSilage.MaizeSilage_Out, feeding_Tank.MaizeSilage_In) annotation(
      Line(points = {{-112, -12}, {-82, -12}, {-82, 4}}));
