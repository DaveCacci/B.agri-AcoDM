...
<p> Last review: <em> December 06, 2020 </em></p></html>"));
  ...
  record ParametersADM1_physiochemical
    ...
    constant Real deltaH0_Ksp_acp(unit = "J/mol") = 22600 "enthalpy of reaction of calcium phosphate formation??; 310 g/mol*26.1 J/g = 8091 J/mol";
...
<p> Last review: <em> December 06, 2020 </em></p></html>"));
  ...
  record StateVariablesADM1
    ...
    Types.MolarDensity_M S_an(fixed = false, start = 0.0197599);                 //chiude initializzazione pH
...
//Gas components
    Modelica.SIunits.MassConcentration S_gas_h2(fixed = true, start = 1.92074e-05);
    Modelica.SIunits.MassConcentration S_gas_ch4(fixed = true, start = 1.48839);
    Types.MolarDensity_M S_gas_co2(fixed = true, start = 0.0144672);
    Types.Pressure_bar p_gas_h2(fixed = false, start = 1.5e-05);
    Types.Pressure_bar  p_gas_ch4(fixed = false, start = 0.582185);
    ...
    Types.MolarDensity_M I(nominal = 1e-1) "mol/L";
    Real g_ca;
    Real g_po4;
    Real g_nh4;
    Real g_mg;
...
//MMP submodel (A =1.82* 10^6 * (ε*T)^(3/2), ε is the dielectric constant of water; Paton et al 2018)
      I = 0.5 * (S_ca * z_ca ^ 2 + S_po4 * z_po4 ^ 2 + S_nh4 * z_nh4 ^ 2 + S_mg * z_mg ^ 2 + S_hpo4 * 2 ^ 2 + S_h2po4 * 1 ^ 2 + S_hco3 * 1 ^ 2 + S_cat * 1 ^ 2 + S_an * 1 ^ 2 + 1 ^ 2 * (S_acm / Ac_conv + S_prom / Pro_conv + S_bum / Bu_conv + S_vam / Va_conv));
...
//Outputs
  ...
  class SynopticModel
    ADM1_P.BlockLibrary.Feed_Source_1 CattleManure annotation(
      Placement(visible = true, transformation(origin = {-122, 48}, extent = {{-18, -18}, {18, 18}}, rotation = 0)));
    ADM1_P.BlockLibrary.Feed_Source_2 CattleSlurry annotation(
      Placement(visible = true, transformation(origin = {-120, 14}, extent = {{-16, -16}, {16, 16}}, rotation = 0)));
    ADM1_P.BlockLibrary.Feed_Source_3 MaizeSilage annotation(
      Placement(visible = true, transformation(origin = {-121, -17}, extent = {{-17, -17}, {17, 17}}, rotation = 0)));
    ADM1_P.BlockLibrary.Feeding_Tank feeding_Tank annotation(
      Placement(visible = true, transformation(origin = {-48, 0}, extent = {{-22, -22}, {22, 22}}, rotation = 0)));
    ...
    ADM1_P.BlockLibrary.Sludge_Discharge Discharge annotation(
      Placement(visible = true, transformation(origin = {92, -17}, extent = {{-30, -30}, {30, 30}}, rotation = 0)));
